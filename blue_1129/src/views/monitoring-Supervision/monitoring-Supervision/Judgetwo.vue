    ...
    <div v-if="!flag">
      <div>
        <div class="table-page-header-submitButtons-back">
          <span style="font-size:17px">分层研判 / 研判二级</span>
        ...
        <div style="height:50px;display:flex;justify-content:space-between">
          <div style="height:50px;display:flex;">
            <a-button
              ...
              :class="changeClass === 0 ? 'ant-btn-primary' : 'ant-btn-default'"
              class="searchBtn"
              @click="judgeNo"
            >未处理</a-button>
            <a-button
              class="searchBtn"
              :class="changeClass === 1 ? 'ant-btn-primary' : 'ant-btn-default'"
              @click="judgeDispose"
            >已处理</a-button>
          </div>
          <div>
        ...
        <a-row :gutter="24">
          <a-col :md="6" :sm="6">
            <a-form-item label="节目名称">
              <a-input style="width:150px;" placeholder="请输入..." v-model.trim="queryParam.name"></a-input>
            </a-form-item>
          </a-col>
          <a-col :md="7" :sm="6">
            <a-form-item label="首次发现网站">
              <a-input style="width:150px;" placeholder="请输入..." v-model.trim="queryParam.source"></a-input>
            </a-form-item>
          </a-col>
          <a-col :md="6" :sm="6">
            <a-form-item label="状态">
              ...
              <!-- <a-input style="width:150px;" placeholder="请输入..." v-model.trim="queryParam.status"></a-input> -->
            </a-form-item>
          </a-col>
          <a-col :md="5" :sm="6">
      ...
      <div>
        ...
        >
          ...
          <div class="geturl" slot="category" slot-scope="category, record">
            <span>{{record.categoryKey}}</span>
          </div>
          <div class="geturl" slot="linkUrl" slot-scope="linkUrl, record">
            <a
              :href="record.programUrl"
              :title="record.programUrl"
              target="_blank"
            >{{record.programUrl}}</a>
          </div>
          <span slot="action" slot-scope="action, record" v-if="showing">
            <a-button @click="dispose(record,'2')">节目处理</a-button>
            <a-divider type="vertical" v-if="hasPerm('enable')" />
            <a-button @click="setNormal(record)" v-if="hasPerm('enable')">设为正常</a-button>
          </span>
          <span v-else>
            <a-button @click="lookShow(record,'2')">查看</a-button>
          </span>
        </a-table>
      </div>
      ...
      <!-- <step-judge-modal :show="flag" @close="close" :reportTemplateId="reportTemplateId" :defaultId="'2'" :item="item" :title="title" :itemList="itemList" /> -->
    </div>
    <stepJudgeDivView2 ref="stepJudgeDivView2" />
    <stepJudgeDiv
      v-if="flag"
      @close="close"
...
import stepJudgeDiv from "./stepJudgeDiv";
import stepJudgeDivView2 from "./stepJudgeDivView2";
import Vue from "vue";
...
export default {
  ...
  data() {
    return {
      ...
      url: {
        list:
          "/nmmonitor-server-module-Judgements/Judgements/queryPorgramVideo",
        delete:
  ...
  methods: {
    ...
    search() {
      ...
      if (this.changeClass === 0) {
        ...
        this.url.list =
          "/nmmonitor-server-module-Judgements/Judgements/queryAuditVideoHisonone";
      }
      this.queryParam.bisType = this.propData.taskBusiness;
      this.queryParam.domType = this.propData.taskDomain;
      this.queryParam.auditLevel = 2;
    ...
    judgeDispose() {
      ...
      query.username = JSON.parse(
        JSON.stringify(Vue.ls.get(USER_INFO).userName)
      );
    ...
    lookShow(record, id) {
      this.title = "查看";
      this.$refs.stepJudgeDivView2.showModal(record);
    },
    explain() {
      let self = this;
      this.$info({
    ...
    checkData(newVal, params) {
      ...
      if (newVal === "2") {
        this.loading = true;
        queryProgramVideo(obj).then(res => {
          if (res.success) {
            this.dataSource = res.result.records;
            this.ipagination.total = res.result.total;
            this.loading = false;

<template>
  <div style="min-height: 100%;background:#fff;padding:24px;border:none;">
      ...
      <div>
        ...
        >
          <span slot="tags" slot-scope="record">
            <a-tag
              :color="record === 0 ? 'orange' : '#2db7f5'"
            >
              {{record === 0 ? '未启用' : '已启用'}}
            </a-tag>
          </span>
          <span slot="action" slot-scope="textCheck, record">
            <a-button type="default" v-if="hasPerm('edit')" @click="editClick(record)" icon="edit">编辑</a-button>
    ...
    <configDiv v-if="flag" :editData="editData" @back="back" />
  </div>
</template>

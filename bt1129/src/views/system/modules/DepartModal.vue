        ...
        <a-spin :spinning="confirmLoading">
            <a-form :form="form">
                <a-form-item
                        ...
                        :hidden="false"
                        hasFeedback>
                    <a-input id="departName" placeholder="请输入机构/部门名称"
    ...
    export default {
        ...
        data() {
            return {
                ...
                url: {
                    add: '/nmmonitor-server/department/addDepartInfo',
                },
            }
        },
        methods: {
            ...
            validateMobile(rule, value, callback) {
                if (!value || new RegExp(/^1([38][0-9]|4[579]|5[0-3,5-9]|6[6]|7[0135678]|9[89])\d{8}$/).test(value)) {
                    callback();
                } else if (!value || new RegExp(/^(\(\d{3,4}\)|\d{3,4}-|\s)?\d{7,14}$/).test(value)) {
                    callback();
                } else {
                    callback("您的电话号码格式不正确!");
                }
            }
        }
    }
